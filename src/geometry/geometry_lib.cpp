...
{
	__m128 _normal = _mm_setr_ps(normal.coordinates[0], normal.coordinates[1],
			normal.coordinates[2], 0.0);

	__m128 _direction = _mm_setr_ps(dir.coordinates[0], dir.coordinates[1],
			dir.coordinates[2], 0.0);
	...
	for (int i = 0; i < polygon.nVertices; ++i)
	{
		...
		__m128 _point = _mm_setr_ps(p.coordinates[0], p.coordinates[1],
				p.coordinates[2], 0.0);
