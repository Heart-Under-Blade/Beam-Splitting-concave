...
HandlerGO::HandlerGO(Scattering *scattering, double wavelength)
...
void HandlerGO::WriteMatricesToFile(std::string &destName)
{
...
{	// OPT: вынести переменные из цикла
	...
	Point3f k = CalcK(tr);
	Point3f n1 = m_particle->elems[tr[0]].actual.in_normal;
...
BeamInfo HandlerGO::ComputeBeamInfo(Beam &beam)
{
	BeamInfo info;
	info.beam = &beam;
