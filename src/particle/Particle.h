...
public:
	...
	void Concatenate(const std::vector<Particle> &parts);
	void RemoveFacet(int index);
	void CommitState();

	void ProjectToXYPlane(std::vector<Point2f> &projection) const;
...
protected:
	...
	void GetFacets(int begin, int end, Array<Facet*> &facets);
